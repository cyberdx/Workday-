...
public class JobRunnerImplTest {
    ...
    public void shouldEventuallyExecuteAllJobs() throws InterruptedException {
        ...
        JobQueue testQueue = new JobQueueImp(jobs);
...
//    @Ignore
    ...
    public void shouldExecuteJobsWithPerformance() throws InterruptedException {
        ...
        for (Job job : jobs) {
            JobImp jobImp = (JobImp) job;
            assertTrue(jobImp.isExecuted());
        }
    }
    ...
    public void shouldExecuteJobsWithFairness() throws InterruptedException {
        ...
        for(Integer customerId: customerIds) {
            for(int i = 1; i <= 1000; i++) {
                jobs.add(new JobImp(customerId, 100));
            }
        }
        ...
        for(Integer customerId : customerIds) {
            int executedJobs = 0;
            for(Job job: jobs) {
                if(((JobImp) job).isExecuted() && job.customerId() == customerId.intValue()) {
                    executedJobs++;
                }
            }
    ...
    public void shouldShutdownGracefully() throws InterruptedException {
        List<Job> jobs = Arrays.asList(new JobImp(), new JobImp(), new JobImp(), new JobImp());
        JobQueue testQueue = new JobQueueImp(jobs);
        JobRunner jobRunner = new JobRunnerImpl();
